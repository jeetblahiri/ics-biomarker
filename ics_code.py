...
def compute_feature_importance(clf, scaler, feature_names):
    # The classifier is fitted on StandardScaler-transformed features, so these
    # coefficients are already standardized and comparable across predictors.
    coef = clf.coef_[0]
    return {name: float(coef[i]) for i, name in enumerate(feature_names)}
...
# ===========================================================================================
# NEW: SAMPLING RATE INVARIANCE ANALYSIS (PARALLELIZED)
# ===========================================================================================


def _compute_single_rate_single_center(args):
    """
    Worker function for parallel sampling rate analysis at a single center.
    
    Parameters:
    -----------
    args : tuple
        (target_fs, records, seg_sec, k_folds, seed, center_name, epoch_class_weight)
    
    Returns:
    --------
    tuple : (target_fs, center_name, auc, n_subjects)
    """
    target_fs, records, seg_sec, k_folds, seed, center_name, epoch_class_weight = args
    rng = np.random.RandomState(seed)
    
    try:
        # Build epoch features at this sampling rate
        X_ics, y_epoch, feature_names, subj_ids_epoch, _ = build_epoch_features(
            records, seg_sec, target_fs=target_fs)
        ...
        if n_subjects < 10:
            return (target_fs, center_name, float("nan"), n_subjects)
        ...
        for fold_idx in range(k_folds):
            ...
            scaler_e = StandardScaler()
            X_train_e = scaler_e.fit_transform(X_ics[train_mask])
            X_val_e = scaler_e.transform(X_ics[val_mask])
            ...
            # Build subject features WITHOUT recording length
            X_subj_train, y_subj_train, _, sids_train = build_subject_features(
                X_ics[train_mask], y_epoch[train_mask], 
                [sid for sid, m in zip(subj_ids_epoch, train_mask) if m], 
                p_train, feature_names,
        ...
        # Compute AUC
        try:
            auc = roc_auc_score(y_subj_true, cv_preds)
        except:
            auc = float("nan")
        
        return (target_fs, center_name, auc, n_subjects)
        
    except Exception as e:
        return (target_fs, center_name, float("nan"), 0)


def _compute_single_rate_pooled(args):
    """
    Worker function for parallel sampling rate analysis on pooled data.
    
    Parameters:
    -----------
    args : tuple
        (target_fs, ihbas_records, max_records, seg_sec, k_folds, seed)
    
    Returns:
    --------
    tuple : (target_fs, "Pooled", auc, n_subjects)
    """
    target_fs, ihbas_records, max_records, seg_sec, k_folds, seed = args
    rng = np.random.RandomState(seed)
    
    try:
        # Build epoch features for both centers
        X_ihbas, y_ihbas, feat_names, sids_ihbas, _ = build_epoch_features(
            ihbas_records, seg_sec, target_fs=target_fs)
        X_max, y_max, _, sids_max, _ = build_epoch_features(
            max_records, seg_sec, target_fs=target_fs)
        ...
        if n_subjects < 10:
            return (target_fs, "Pooled", float("nan"), n_subjects)
        ...
        for fold_idx in range(k_folds):
            ...
            scaler_e = StandardScaler()
            X_train_e = scaler_e.fit_transform(X_all[train_mask])
            X_val_e = scaler_e.transform(X_all[val_mask])
        ...
        return (target_fs, "Pooled", auc, n_subjects)
        
    except Exception as e:
        return (target_fs, "Pooled", float("nan"), 0)


def run_sampling_rate_analysis(ihbas_records: List[SubjectRecord], 
                                max_records: List[SubjectRecord],
                                seg_sec: float, k_folds: int, 
                                seed: int, output_dir: Path,
                                n_workers: int = None):
    """
    Analyze biomarker performance across different sampling rates for IHBAS, MAX, and Pooled.
    
    This tests whether the ICS biomarker is robust to variations in EEG sampling rate,
    which is important for clinical deployment across different acquisition systems.
    
    Parameters:
    -----------
    ihbas_records : list of SubjectRecord
        IHBAS subject records
    max_records : list of SubjectRecord
        MAX subject records
    seg_sec : float
    ...
    Returns:
    --------
    dict : Results for each center at each sampling rate
    """
    print("\n" + "="*80)
    print("SAMPLING RATE INVARIANCE ANALYSIS (PARALLELIZED)")
    print("="*80)
    
    if n_workers is None:
        n_workers = min(multiprocessing.cpu_count(), 16)
    
    # Target sampling rates to test (Hz)
    target_rates = [125, 100, 80, 60, 40, 20, 10, 5, 4, 3, 2, 1]
    
    print(f"Testing {len(target_rates)} sampling rates: {target_rates}")
    print(f"Using {n_workers} parallel workers")
    
    # Prepare tasks for single-center analyses
    tasks_ihbas = [(fs, ihbas_records, seg_sec, k_folds, seed, "IHBAS", None) 
                   for fs in target_rates]
    tasks_max = [(fs, max_records, seg_sec, k_folds, seed, "MAX", "balanced") 
                 for fs in target_rates]
    tasks_pooled = [(fs, ihbas_records, max_records, seg_sec, k_folds, seed) 
                    for fs in target_rates]
    ...
    # Run single-center analyses
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        # Submit IHBAS tasks
        futures_ihbas = {executor.submit(_compute_single_rate_single_center, task): task 
                        for task in tasks_ihbas}
        # Submit MAX tasks
        futures_max = {executor.submit(_compute_single_rate_single_center, task): task 
                      for task in tasks_max}
        # Submit Pooled tasks
        futures_pooled = {executor.submit(_compute_single_rate_pooled, task): task 
                         for task in tasks_pooled}
        
        all_futures = {**futures_ihbas, **futures_max, **futures_pooled}
        ...
        for future in as_completed(all_futures):
            completed += 1
            try:
                target_fs, center_name, auc, n_subjects = future.result()
                results[center_name][target_fs] = {"auc": auc, "n_subjects": n_subjects}
                print(f"  [{completed}/{total}] {center_name} @ {target_fs} Hz: AUC = {auc:.3f}")
            except Exception as e:
                task = all_futures[future]
                print(f"  [{completed}/{total}] Error: {e}")
    
    # Create plot
    plot_sampling_invariance_multicenter(results, output_dir)
    
    # Print summary
    print("\n" + "-"*70)
    print("SAMPLING RATE INVARIANCE SUMMARY")
    print("-"*70)
    print(f"{'Rate (Hz)':<12} {'IHBAS AUC':<12} {'MAX AUC':<12} {'Pooled AUC':<12}")
    print("-"*70)
    for fs in sorted(target_rates, reverse=True):
        ihbas_auc = results["IHBAS"].get(fs, {}).get("auc", float("nan"))
        max_auc = results["MAX"].get(fs, {}).get("auc", float("nan"))
        pooled_auc = results["Pooled"].get(fs, {}).get("auc", float("nan"))
        print(f"{fs:<12} {ihbas_auc:<12.3f} {max_auc:<12.3f} {pooled_auc:<12.3f}")
    print("-"*70)
    
    return results


def plot_sampling_invariance_multicenter(results: Dict, output_dir: Path):
    """
    Plot AUC vs sampling rate for IHBAS, MAX, and Pooled on the same figure.
    
    Parameters:
    -----------
    results : dict
        Results dictionary with keys "IHBAS", "MAX", "Pooled"
    output_dir : Path
        Directory for saving the plot
    """
    # Extract data for each center
    centers = ["IHBAS", "MAX", "Pooled"]
    colors = {"IHBAS": '#1f77b4', "MAX": '#ff7f0e', "Pooled": '#2ca02c'}
    markers = {"IHBAS": 'o', "MAX": 's', "Pooled": '^'}
    ...
    for center in centers:
        ...
        center_results = results[center]
        rates = sorted([r for r in center_results.keys() 
                       if not np.isnan(center_results[r].get("auc", float("nan")))], reverse=True)
        aucs = [center_results[r]["auc"] for r in rates]
    ...
    # Add reference lines
    ax.axhline(y=0.7, color='gray', linestyle='--', alpha=0.7, linewidth=1.5)
    ...
    # Reverse x-axis to show high rates on left (like your image)
    ax.invert_xaxis()
    ax.set_xlim(130, 0)
    ax.set_ylim(0.5, 1.0)
    
    # Custom x-ticks
    all_rates = [125, 100, 80, 60, 40, 20, 10, 5, 4, 3, 2, 1]
    ax.set_xticks(all_rates)
    ax.set_xticklabels([str(r) for r in all_rates])
    ...
    # Save plot
    save_path = output_dir / "sampling_invariance_auc.png"
...
def run_recording_length_analysis(ihbas_root, max_root, seg_sec, k_folds, n_boot, output_dir, rng):
    ...
    # =========================================================================
    # 4. Sampling Rate Invariance Analysis (Parallelized)
    # =========================================================================
    sampling_results = run_sampling_rate_analysis(
        ihbas_records, max_records, seg_sec, k_folds, 
        seed=0, output_dir=output_dir)
    
    results_summary["sampling_rate_invariance"] = sampling_results
...
def main():
    ...
    ap.add_argument("--output_dir", type=str, default="ics_results_samplingrate")
    ap.add_argument("--run_length_controls", action="store_true",
                    help="Run comprehensive recording length control analyses")
    ap.add_argument("--run_sampling_analysis", action="store_true",
                    help="Run sampling rate invariance analysis only")
    ap.add_argument("--n_workers", type=int, default=None,
                    help="Number of parallel workers for sampling rate analysis")
    args = ap.parse_args()
    ...
    # Run standalone sampling rate analysis if requested
    if args.run_sampling_analysis:
        ihbas_root = Path(args.ihbas_root).expanduser().resolve()
        max_root = Path(args.max_root).expanduser().resolve()
        ...
        results = run_sampling_rate_analysis(
            ihbas_records, max_records, args.seg_sec, args.k_folds,
            seed=args.seed, output_dir=output_dir, n_workers=args.n_workers)
    else:
        run_full_analysis(
